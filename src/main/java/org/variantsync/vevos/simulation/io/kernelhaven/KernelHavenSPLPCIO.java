 ...
 * The annotated code will be considered to have inline annotations (in CPP
 * style):
 * #if
 * foo();
 * #endif
 * This importer specifically expects the imported files to be produced by
 * KernelHaven to adjust for its quirks.
 */
public class KernelHavenSPLPCIO extends KernelHavenPCIO {
    ...
    protected LineBasedAnnotation createAnnotation(final Node blockCondition, final Node presenceCondition,
        ...
        /// If a block starts at 1 in KernelHaven files, it does not denote an #if but
        /// the entire file.
        /// Thus, there is no #if at line 1 but LineBasedAnnotation expects a macro at
        /// startLine.
        final boolean isVirtualSurroundingTrue = startLine == 1;
        /*
         * Line numbers for the all surrounding true are given in [1, last line of file]
         * format.
