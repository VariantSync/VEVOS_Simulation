...
import org.variantsync.vevos.simulation.util.StackUtil;
...
public class Domino implements SequenceExtractor {
     ...
     * Each sublist corresponds to a successive chains of commits that directly
     * follow each other.
     */
    @Override
    public List<NonEmptyList<SPLCommit>> extract(final Collection<SPLCommit> commits) {
        final Stack<Stack<SPLCommit>> dominos = Sort(VariabilityDataset.streamEvolutionSteps(commits)::iterator);
    ...
    public static <C extends Commit> Stack<Stack<C>> Sort(final Iterable<EvolutionStep<C>> dominos) {
         ...
         * For each domino, we sort it into a chain of other dominos if there is a
         * suitable chain.
