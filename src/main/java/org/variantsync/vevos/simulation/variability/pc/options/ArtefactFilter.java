...
public interface ArtefactFilter<A extends Artefact> {
    /**
     * Determines whether a given artefact should be considered in or dropped from
     * subsequent computations.
     * 
     * @param a The artefact that should be filtered.
     * @return True iff the given artefact should be kept and used in further
     *         computations.
     */
    boolean shouldKeep(final A a);

    /**
     * @return A filter that accepts all artifacts. Formally, its value is `a ->
     *         true`.
     */
     ...
     * The returned filter will run all filters in the order they are returned by
     * the given collection.
     * The returned filter returns false and stops computation as soon as one of the
     * given filters
     ...
     * @return A single filter that returns true for a given artifact iff all of the
     *         given filters return true for it.
     *         Formally, if `f` is the output filter `f.shouldKeep(a) iff (forall fi
     *         in filters: fi.shouldKeep(a))`.
     */
