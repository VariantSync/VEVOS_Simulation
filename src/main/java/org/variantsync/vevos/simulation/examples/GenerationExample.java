...
public class GenerationExample {
    public static void main(final String[] args) throws Resources.ResourceIOException {
        ...
        final CaseSensitivePath variantsGenerationDir = CaseSensitivePath.of("directory", "to", "put", "generated",
                "variants");

        /// Load the ground truth dataset extracted with the variability extraction of
        /// VEVOS
        ...
        /// The history is divided into sub-histories because for some commits in the
        /// SPL, the
        /// commit extraction might have failed.
        /// If the extraction fails for a commit c, then we have to exclude c from the
        /// variant
        /// generation.
        ...
        /// In case the sequence extraction fails, you might either use the
        /// evolutionSteps directly
        /// or
        ...
        /// First, create a sampler that determines which variants to generate at each
        /// evolution
        /// step.
        ...
        /// Busybox requires special pre- and postprocessing steps upon commit checkout,
        /// that are
        /// handled by the
        ...
        for (final NonEmptyList<SPLCommit> subhistory : history.commitSequences()) {
            for (final SPLCommit splCommit : subhistory) {
                ...
                final Lazy<Optional<Artefact>> loadPresenceConditions = splCommit.presenceConditionsFallback();
                ...
                /// Let's sample the set of variants we want to generate for the feature model
                /// at
                /// this commit.
                ...
                /// For example, a study could be interested only in generating the changed
                /// files in
                /// a commit.
                /// In our case, let's just generate all variants.
                /// Moreover, VariantGenerationOptions allow to configure some parameters for
                /// the
                /// variant generation.
                ...
                final VariantGenerationOptions generationOptions = VariantGenerationOptions.ExitOnError(false,
                        artefactFilter);
                ...
                if (splRepository instanceof BusyboxRepository b) {
                    try {
                        ...
                        Logger.error(
                                "Busybox postprocessing failed, please clean up manually (e.g., git stash, git stash drop) at "
                                        + splRepository.getPath(),
                                e);
                    }
                }
            }
