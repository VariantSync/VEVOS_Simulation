...
/**
 * Default blueprint to generate variants from the software product line at a
 * certain commit.
 */
public class VariantsRevisionFromVariabilityBlueprint extends VariantsRevisionBlueprint {
    ...
    /**
     * Creates a new blueprint that can generate variants from the given splCommit
     * that contains the
     * variability information of a specific SPLCommit.
     * 
     * @param splCommit   The variability commit from which a VariantsRevision
     *                    should be created.
     * @param predecessor The predecessor blueprint that generates the
     *                    VariantsRevision that has to
     *                    be generated before the revision of this blueprint. May be
    ...
    public Lazy<VariantsRevision.Branches> generateArtefactsFor(final VariantsRevision revision) {
        return splCommit.presenceConditionsFallback().and(getSample()).map(ts -> {
            // TODO: Should we implement handling of an empty optional, or do we consider
            // this to be
            // a fundamental error?
            ...
            for (final Variant variant : sample.variants()) {
                ...
                // Commit the generated variant with the corresponding spl commit has as
                // message.
