 ...
 * For instance, when there would be a single commit in the mid of the history
 * of the ISPLRepository,
 * that could not be parsed (we have no feature model and no presence
 * conditions), this commit would split
 * the history of the ISPLRepository we can model in half - all commits before
 * that error commit and all commits afterwards.
 * A VariantsRevisionFromErrorBlueprint is devoted to model such an error commit
 * on the AbstractVariantsRepository to mark
...
public class VariantsRevisionFromErrorBlueprint extends VariantsRevisionBlueprint {
    ...
    protected Lazy<Sample> computeSample() {
        // We don't have any variability information but instead want to introduce an
        // artificial error commit.
        // Thus, we just have to operate on the variants already present.
        // We know that we have a predecessor because it is necessary to have one in the
        // constructor.
        ...
        return getSample().map(sample -> {
            ...
            for (final Variant variant : sample.variants()) {
                ...
                try {
                    ...
                    // TODO: We cannot commit no changes. So we have to change something. What could
                    // that be?
                    // A simple text file might really be all we need here. Either an empty file or
                    // a file with the hashes of the associated commits.
                    // Idea: Remember in the error blueprint the number of the sub-history it ends.
                    // As initial commit to each branch,
