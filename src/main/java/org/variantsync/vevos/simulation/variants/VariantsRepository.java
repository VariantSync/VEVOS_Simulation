 ...
 * Given a history of VariantsRevisionBlueprints, this class will generate a git
 * repository with variants from
...
public class VariantsRepository extends AbstractVariantsRepository {
     ...
     * 
     * @param localPath        The directory where the VariantsRepository is already
     *                         located or should be located.
     * @param splRepo          The ISPLRepository from whose artefacts, variants
     *                         should be generated.
     * @param blueprintHistory A history of blueprints on how to generate the
     *                         variants.
     *                         The variants revisions will be generated in the order
     *                         the blueprints appear in the list.
     */
    public VariantsRepository(
        ...
        final ListHeadTailView<VariantsRevisionBlueprint> history = filterExistingRevisions(
                new ListHeadTailView<>(blueprintHistory));
     ...
     * If it does, reads all relevant metadata from it, such as the currently
     * checked out commit and the existing branches.
     */
    private void parseRepoMetadata() {
        // TODO: Implement Issue 11 here.
        // If we start repo generation from the beginning, nothing has to be done, thus
        // we have no branches in the beginning:
        branchesByName = new HashMap<>();
    }

    /**
     * Discards all blueprints from the given history for which revisions where
     * already generated.
     * This means, this removes blueprints from the head of the given history until
     * the current head was not already generated.
     * 
     ...
     */
    private ListHeadTailView<VariantsRevisionBlueprint> filterExistingRevisions(
            final ListHeadTailView<VariantsRevisionBlueprint> history) {
        // TODO: Implement Issue 11 here.
        // E.g. if we see that the first blueprint was already processed then we could
        // return history.tail().
        // 1.) Find the last valid revision.
        // 2.) If there were commits after that revision they were incomplete (e.g., we
        // generated some but not all variants).
        // Thus, delete all changes after the last valid revision. This might require
        // manipulating the git history.
        // 3.) Cut the first n blueprints in `history` such that in the end
        // `history.head()` is the first blueprint whose revision has to be generated.
        // (The blueprint before `history.head()` would be the blueprint that produced
        // the last valid revision from 1).
        return history;
    }
    ...
    // (At the moment it uses the current branch because it is only used for
    // getCurrentCommit)
    ...
    // At the moment, it returns null in the case of an empty commit, and the
    // VariantCommit otherwise (as described in AbstractVariantRepository)
    // But in VariantsRevisionFromErrorBluePrint, an empty commit would lead to
    // throwing a RuntimeException. Is that wanted?
    // If empty commits should be possible, commit() could be adapted to allow
    // those. (referring to TODO note in VariantsRevisionFromErrorBlueprint)
