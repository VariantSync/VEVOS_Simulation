...
public interface IRepository<C extends Commit> extends AutoCloseable {
     ...
     * 
     * @param c The commit to check out. Will be the @see
     *          getCurrentCommitAfterwards.
     ...
     * @param c      The commit to check out. Will be the @see
     *               getCurrentCommitAfterwards.
     * @param forced If true, allow a checkout even if the workingtree or index
     *               differs from HEAD.
     * @return The commit that was previously the current commit.
     */
    ...
    /**
     * @return The commit HEAD is currently pointing to (i.e., the commit whose
     *         state of the repository we see on disk currently).
     */
