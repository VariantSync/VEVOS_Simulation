...
import de.variantsync.evolution.util.fide.bugfix.FFalse;
import de.variantsync.evolution.util.fide.bugfix.FTrue;
...
public class PCLoaderTest {
    ...
    public void loadTestFileCorrectly() {
        ...
        { // Build the expected result by hand.
            final SourceCodeFile alex = new SourceCodeFile(Path.of("src", "Alex.cpp"), FTrue.asLiteral());
            {
                PreprocessorBlock a = new PreprocessorBlock(new Literal("A"), 30, 60);
                a.addBlock(new PreprocessorBlock(new Literal("B"), 50, 55));
                PreprocessorBlock tru = new PreprocessorBlock(FTrue.asLiteral(), 1, 100);
                tru.addBlock(a);
            ...
            {
                bar.addBlock(new PreprocessorBlock(FFalse.asLiteral(), 1, 20));
            }

            expectedResult = new PresenceConditions(Arrays.asList(alex, bar));
        }

        assert expectedResult.equals(result.getSuccess());
    }
}
